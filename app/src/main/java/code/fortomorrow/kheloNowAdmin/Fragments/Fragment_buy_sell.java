...
public class Fragment_buy_sell extends Fragment implements Buy_sell_adapter.OnDeleteItemClickListener, Buy_sell_adapter.OnItemClickListener {
    ...
    private void show_item() {
        apiService.getBuySellItem(secret_id, api_token, "buy_sell").enqueue(new Callback<Buy_sell_response>() {
            @Override
            public void onResponse(Call<Buy_sell_response> call, Response<Buy_sell_response> response) {
                if (response.body().getE() == 0 && response.body().getM().size() != 0) {
                    ...
                    buy_sell_adapter.setOnClickListener(Fragment_buy_sell.this::OnItemClick, Fragment_buy_sell.this::OnDeleteItemClick);
